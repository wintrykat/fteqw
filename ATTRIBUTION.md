...
- `docs/PORTING.md`, `docs/BUILDING.md` — change/workaround documentation and build guide
- `macos/scripts/*` — reproducible build, packaging, and self-contained bundling
- `macos/tests/*` — the automated test suite
...
The distributed `.app` bundles the following libraries, obtained via Homebrew,
each under its own license (all GPL-compatible): SDL (sdl2-compat, SDL3),
MoltenVK (Apache-2.0), FreeType (FTL/GPL), libpng, jpeg-turbo, libogg, libvorbis,
Opus, Speex, and FFmpeg (LGPL/GPL depending on build). Their source is available
from their respective projects and from Homebrew.
